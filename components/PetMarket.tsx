...
export function PetMarket({
  ...
  const getAllListedNFTs = useCallback(async (contract, nft_contract, chainId) => {
    try {
  ...
  const handleBuyPet = async (pet: Pet) => {
    ...
    // const signer = await ethersProvider.getSigner();
      ...
      const provider = ethers.getDefaultProvider()
      const signer = new ethers.Wallet(privateKey, provider);
    const nftGeneratorContraact = new ethers.Contract(NftGeneratortAddress.address, NftGeneratorABI, signer) 
